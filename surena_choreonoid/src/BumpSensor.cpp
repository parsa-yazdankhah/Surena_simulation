...
BumpSensor::BumpSensor(ros::NodeHandle* nh, double max_h, char* simulator){
    ...
    if(strcmp(simulator,"bullet") == 0){
        sole_pos.open("/config/sole_bullet.txt");
        cout << "Using Dimentions for Bullet Simulation" << endl;
    }
    else{
        sole_pos.open("/config/sole_choreonoid.txt");
...
bool BumpSensor::sensorCallback(surena_choreonoid::bump::Request  &req,surena_choreonoid::bump::Response &res){
